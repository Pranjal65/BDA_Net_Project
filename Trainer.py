...
class EMAWeightOptimizer(object):
    ...
    def __init__(self, target_net, source_net, ema_alpha):
        ...
        self.target_params0 = [
            p for p in target_net[0].state_dict().values() if p.dtype == torch.float
        ]
        self.target_params1 = [
            p for p in target_net[1].state_dict().values() if p.dtype == torch.float
        ]
        self.source_params = [
            p for p in source_net.state_dict().values() if p.dtype == torch.float
        ]
...
def cal_category_confidence(
    ...
    for ind in range(num_classes):
        ...
        if torch.sum(cat_mask_sup_gt) == 0:
            ...
            value = torch.sum(conf_map_sup * cat_mask_sup_gt.float()) / (
...
class Criterion_cons(nn.Module):
    def __init__(self, gamma, sample=False, gamma2=1, ignore_index=255):
        """
...
class Trainer(object):
    ...
    def __init__(
        ...
        if self.config.use_seed:
            ...
            self.checkpoint_dir = os.path.join(
                self.args.output, self.model_name, self.begin_time
            )
            self.log_dir = os.path.join(
                self.args.output, self.model_name, self.begin_time, "log"
            )
        ...
        self.optimizer_teacher = EMAWeightOptimizer(
            self.model_teacher, self.model_student, 0.99
        )
        ...
        if self.config.use_one_cycle_lr:
            ...
            self.lr_scheduler = self._lr_scheduler_lambda(
                self.optimizer_student, last_epoch=self.start_epoch - 1
            )
    ...
    def _optimizer(self, lr_algorithm):
        ...
        if lr_algorithm == "adam":
            optimizer = optim.Adam(
                filter(lambda p: p.requires_grad, self.model_student.parameters()),
                lr=self.current_lr,
                betas=(0.9, 0.999),
                eps=1e-08,
    ...
    def _lr_scheduler_lambda(self, optimizer, last_epoch):
        ...
        lr_scheduler = optim.lr_scheduler.LambdaLR(
            optimizer, lr_lambda=lambda1, last_epoch=last_epoch
        )
    ...
    def train_damage_PDMT(self):
        ...
        for epoch in range(self.start_epoch, epochs + 1):
            ...
            train_log, class_criterion = self._train_epoch_damage_PDMT(
            ...
            if self.monitor_mode != "off":
                improved = (
                    ...
                    and eval_log["val_" + self.monitor_metric] < self.monitor_best
                ) or (
                ...
                if self.not_improved_count > self.early_stop:
                    print(
                        ...
                        "     + Training stop :/".format(self.not_improved_count)
                    )
                    break
    ...
    def _train_epoch_damage_PDMT(self, epoch, class_criterion):
        ...
        cons_loss = Criterion_cons(
            gamma=self.config.gamma, sample=False, gamma2=1, ignore_index=255
        )
        ...
        if display:
            ...
            print(
                "epoch [", epoch, ": ]", "sample_rate_target_class_conf", conf_print
            )  # sample rate
        ...
        max_samples = (
            max(len(self.train_data_loader), len(self.unsupervised_train_loader_0))
        ...
        for idx in pbar:
            train_minibatch = next(train_dataloader)
            ...
            with autocast():
                ...
                sup_BD_loss = bce_loss(
                    torch.squeeze(sup_logits["building"], dim=1), target_BD
                )
            ...
            with torch.no_grad():
                ...
                class_criterion = (
                    ...
                    + category_entropy * (1 - class_momentum)
                )
            ...
            if epoch > self.config.warmup_period:
                ...
                if self.config.use_cutmix:
                    batch_mix_masks = torch.from_numpy(
                        mask_generator.generate_params(
                            unsup_imgs_0.shape[0],
                            (self.config.input_size, self.config.input_size),
                        ).astype(dtype=np.float32)
                    ).to(self.device, non_blocking=True)
                    unsup_imgs_mixed = (
                        unsup_imgs_0 * (1 - batch_mix_masks)
                        + unsup_imgs_1 * batch_mix_masks
                    )
                ...
                with torch.no_grad():
                    ...
                    prob_u0_tea_BD_1 = torch.sigmoid(
                        logits_u0_tea_1["building"]
                    ).detach()
                    ...
                    if self.config.use_mix:
                        ...
                        prob_u1_tea_BD_1 = torch.sigmoid(
                            logits_u1_tea_1["building"]
                        ).detach()
                        prob_u1_tea_D_1 = torch.sigmoid(
                            logits_u1_tea_1["damage"]
                        ).detach()
                        prob_cons_tea_BD_1 = (
                            prob_u0_tea_BD_1 * (1 - batch_mix_masks)
                            + prob_u1_tea_BD_1 * batch_mix_masks
                        )
                        prob_cons_tea_D_1 = (
                            prob_u0_tea_D_1 * (1 - batch_mix_masks)
                            + prob_u1_tea_D_1 * batch_mix_masks
                        )
                    else:
                ...
                with autocast():
                    ...
                    unsup_loss_BD = bce_loss(
                        torch.squeeze(logits_cons_stu_1["building"], dim=1),
                        ps_label_BD_1,
                    )
                    unsup_loss_D = cons_loss(
                        logits_cons_stu_1["damage"],
                        conf_unsup1,
                        ps_label_D_1,
                        class_criterion[0],
                    )
        ...
        fwiou0 = (
            self.evaluator_BD.Frequency_Weighted_Intersection_over_Union()
            .cpu()
            .detach()
            .numpy()
        )
        confusion_matrix0 = (
            self.evaluator_BD.get_confusion_matrix().cpu().detach().numpy()
        )
    ...
    def _eval_epoch_damage_PDMT(self, epoch):
        ...
        with torch.no_grad():
            for steps, (imgs, gts, filename) in enumerate(
                self.valid_data_loder, start=1
            ):
                ...
                sup_logits = {}
                sup_logits["damage"] = (
                    sup_logits0["damage"] + sup_logits1["damage"]
                ) / 2
                sup_logits["building"] = (
                    sup_logits0["building"] + sup_logits1["building"]
                ) / 2
            ...
            acc_class0 = self.evaluator_BD.Pixel_Accuracy_Class().cpu().detach().numpy()
            miou0 = (
                self.evaluator_BD.Mean_Intersection_over_Union().cpu().detach().numpy()
            )
            fwiou0 = (
                self.evaluator_BD.Frequency_Weighted_Intersection_over_Union()
                .cpu()
            ...
            confusion_matrix1 = (
                self.evaluator.get_confusion_matrix().cpu().detach().numpy()
            )
            TP, FP, FN, TN = self.evaluator.get_base_value()
            iou = self.evaluator.get_iou().cpu().detach().numpy()
            prec = self.evaluator.Pixel_Precision_Class().cpu().detach().numpy()
            ...
            print(
                ...
                "Precision: {:6.4f},  Recall: {:6.4f},      F_Score: {:6.4f}".format(
                    self.current_lr,
                    miou,
                    acc,
    ...
    def _save_ckpt(self, epoch, best):
        ...
        filename = os.path.join(
            self.checkpoint_dir, "checkpoint-ep{}.pth".format(epoch)
        )
        best_filename = os.path.join(self.checkpoint_dir, "checkpoint-best.pth")
        last_best_filename = os.path.join(
            self.checkpoint_dir,
        ...
        if best:
            ...
            print(
                "     + Saving Best Checkpoint : Epoch {}  path: {} ...  ".format(
                    epoch, best_filename
                )
            )
            ...
            if epoch > start_save_epochs:
                print(
                    "     + After {} epochs, saving Checkpoint per {} epochs, path: {} ... ".format(
                        start_save_epochs, self.save_period, filename
                    )
                )
    ...
    def _resume_ckpt_PSMT(self, resume_file):
        ...
        print(
            ...
            "     + Prepare to test ! ! !".format(resume_file)
        )
